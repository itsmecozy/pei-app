...
export function EthicsPage() {
  ...
  const principles = [
    { icon:"◎", title:"No account required",           body:"You do not need an account to submit. Open to any Filipino with a phone or browser.",                                                                                                                     color:T.teal    },
    { icon:"◌", title:"What we store",                 body:"Your city, your emotion, and a timestamp. Nothing else is recorded. A daily limit per device keeps results balanced — we use a hashed key that resets at midnight.",                                     color:T.amber   },
    ...
    { icon:"◷", title:"What this is not",              body:"PEI is not a poll and it is not representative. Participation is self-selected. Cities with more internet access will always be louder. Read it as a mood signal, not a measurement.",                   color:"#10b981" },
...
export function MethodologyPage() {
  ...
  const [ref, inView] = useInView();
  const sections = [
    { icon:"◈", title:"What is PEI?",             color:T.amber,   body:"The Philippines Emotional Index is a voluntary emotional census. It collects self-reported emotions from Filipinos across 1,634 cities and municipalities and aggregates them into a real-time national pulse. It is not a scientific survey. It does not claim statistical representativeness. It is a mirror — reflecting what participating Filipinos choose to share." },
    ...
    { icon:"✓", title:"What data does PEI collect?", color:"#10b981", body:"We store a city code, an emotion, and a timestamp — nothing else. No name, no account, no IP address in readable form. A daily limit is enforced using a hashed device key that resets at midnight. The smallest unit of display is always a community — a city must reach 50 submissions before appearing on the map." },
  ];
