...
function DocPage({ label, title, subtitle, sections }) {
  ...
  return (
    <div style={{ padding:bp==="mobile"?"0 1.25rem":"0" }}>
      ...
      {sections.map((s, i) => (
        <div key={i} style={{ marginBottom:"2rem", paddingBottom:"2rem",
          borderBottom:`1px solid ${T.border}` }}>
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"1.1rem",
            fontWeight:700, marginBottom:"0.75rem", letterSpacing:"-0.01em" }}>{s.title}</h2>
          <div style={{ fontFamily:"DM Mono", fontSize:"0.63rem", color:T.muted, lineHeight:1.9 }}>
            {s.body}
          </div>
        </div>
      ))}
...
function PrivacyPolicyPage() {
  return <DocPage label="Legal" title="Privacy Policy."
    subtitle="How the Philippines Emotional Index handles — and deliberately does not handle — your data."
    sections={[
      { title:"1. What We Collect", body:"When you submit a feeling, we record: the emotional category you selected, the intensity level (1–5), the LGU you identified, an optional short-form text message if you chose to include one, and a one-way hash of a session token that expires within 24 hours. We do not collect your IP address in any persistent form, and we do not collect your name, email, phone number, or any account identifier." },
      { title:"2. What We Do Not Collect", body:"We do not collect device fingerprints, browser identifiers, geolocation coordinates, behavioral data, cookies for tracking, or any cross-session identity signal. The system is designed so that even a compelled disclosure would yield no individually identifying information." },
      { title:"3. How Data Is Stored", body:"Raw individual submissions are retained for a maximum of 72 hours before being collapsed into community-level statistics. Optional text messages are processed for emotional signal classification and then immediately discarded — the raw text is never written to permanent storage." },
      { title:"4. How Data Is Displayed", body:"No individual submission is ever displayed. Community data is only shown when a minimum of 50 submissions from that LGU has been reached. The smallest unit of display is always a community, never a person." },
      { title:"5. Third Parties", body:"We do not sell, license, share, or transfer your data to any third party for any commercial purpose. Aggregated, anonymized statistics may be shared with academic research partners under a formal data access agreement." },
      { title:"6. Your Rights", body:"Because we do not collect identifying information, we cannot retrieve or delete a specific individual's submission on request — there is no linkage between a submission and a person. This is a feature of the design, not a limitation of our policy." },
      { title:"7. Changes to This Policy", body:"We will post any changes to this privacy policy on this page with a revised effective date. Continued use of the submission feature constitutes acceptance of the updated policy. Effective date: January 1, 2025." },
    ]} />;
}
  ...
  return (
    <div style={{ padding:bp==="mobile"?"0 1.25rem":"0" }}>
        ...
        {layers.map((l, i) => (
          <div key={i} style={{ background:T.bg, padding:"1.5rem",
            ...
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"2.5rem",
              color:l.color, opacity:0.12, lineHeight:1, marginBottom:"0.5rem" }}>{l.num}</div>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"0.95rem",
      ...
      <div style={{ border:`1px solid ${T.teal}20`, background:`${T.teal}05`,
        ...
        <span style={{ color:T.teal }}>Design Principle: </span>
        Anonymity by architecture means that even if PEI were legally compelled to disclose user data, the disclosed data would contain no information that could identify an individual.
      </div>
    </div>
  );
}
...
function DataMethodologyPage() {
  return <DocPage label="Legal" title="Data Methodology."
    subtitle="A technical specification of how emotional signals are collected, weighted, and aggregated into the PEI index."
    sections={[
      { title:"Signal Collection", body:"Each submission captures: emotion_key (one of 8 canonical emotions), intensity (integer 1–5), lgu_id (foreign key to the LGU registry), and submitted_at (UTC timestamp). Optional: text_signal (string, max 150 chars, processed then discarded)." },
      { title:"Weighting Model", body:"PEI uses an intensity-weighted frequency model: each submission's contribution to its emotion bucket is multiplied by its intensity score divided by 5. A submission of Grief at intensity 5 contributes 1.0 units; at intensity 2 it contributes 0.4 units. Buckets are then normalized to percentages." },
      { title:"Community Thresholds", body:"A minimum of 50 submissions is required before a community's data is surfaced publicly. This threshold ensures statistical stability and prevents de-anonymization through inference from small groups." },
      { title:"Dominance Calculation", body:"The dominant emotion is the emotion with the highest intensity-weighted frequency score after normalization. Ties are broken by recency — the emotion with the more recent submission median is preferred." },
      { title:"Time Windows", body:"Three time windows are maintained: Live (rolling 24 hours), Weekly (rolling 7 days), and All-Time. Each window is calculated independently on a 15-minute refresh cycle." },
      { title:"Known Limitations", body:"Self-selection bias: participants choose to submit, and emotionally activated individuals are more likely to participate. Platform bias: mobile and desktop internet access skews submissions toward urban, younger, and more connected populations. PEI is a mirror, not a census." },
    ]} />;
}

function ResearchAccessPage() {
  return <DocPage label="Legal" title="Research Access."
    ...
    sections={[
      { title:"Who May Apply", body:"Research access is available to faculty members, postdoctoral researchers, and graduate students affiliated with accredited universities or research institutions. Independent researchers may apply on a case-by-case basis. Commercial entities are not eligible for the research access tier." },
      { title:"What Is Available", body:"Approved researchers receive access to: anonymized community-level aggregate exports (LGU × emotion × intensity × time window), historical trend data with configurable time resolution (daily, weekly, monthly), and regional breakdowns by province and island group." },
      { title:"Application Process", body:"Submit a research proposal via the Research Portal that includes: institutional affiliation, research question and methodology, intended use of data, data security plan, and expected publication timeline. Applications are reviewed on a rolling basis with a typical 2–4 week turnaround." },
      { title:"Data Use Restrictions", body:"Research data may not be: shared with non-approved parties, used for commercial purposes, used to attempt to re-identify individuals, or published in ways that present PEI data as a representative scientific sample without appropriate methodological caveats." },
      { title:"Attribution", body:'Publications using PEI data should cite: "Philippines Emotional Index (PEI), [year]. Anonymized community-level aggregate data. pei.ph." We request a pre-publication notification and a copy of any paper that uses PEI data.' },
    ]} />;
}
...
function ContactPage() {
  ...
  const Field = ({ label, k, placeholder, multi }) => (
    <div style={{ marginBottom:"1rem" }}>
      <label style={{ fontFamily:"DM Mono", fontSize:"0.52rem", letterSpacing:"0.12em",
        textTransform:"uppercase", color:T.muted, display:"block", marginBottom:"0.35rem" }}>{label}</label>
      {multi
        ? <textarea value={form[k]} onChange={e=>set(k,e.target.value)} placeholder={placeholder} rows={5}
            style={{ width:"100%", background:T.bg, border:`1px solid ${T.border}`, color:T.text,
              padding:"0.6rem 0.8rem", fontFamily:"DM Mono", fontSize:"0.65rem", outline:"none",
              resize:"vertical", lineHeight:1.6, boxSizing:"border-box" }}
            onFocus={e=>e.target.style.borderColor=T.amber}
            onBlur={e=>e.target.style.borderColor=T.border} />
        : <input value={form[k]} onChange={e=>set(k,e.target.value)} placeholder={placeholder}
            style={{ width:"100%", background:T.bg, border:`1px solid ${T.border}`, color:T.text,
              padding:"0.6rem 0.8rem", fontFamily:"DM Mono", fontSize:"0.65rem",
  ...
  return (
    <div style={{ padding:bp==="mobile"?"0 1.25rem":"0" }}>
      ...
      <div style={{ display:"grid",
        gridTemplateColumns:bp==="desktop"?"1fr 1fr":"1fr", gap:"3rem", marginBottom:"3rem" }}>
        <div>
          <Field label="Your Name"     k="name"    placeholder="Maria Santos" />
          <Field label="Email Address" k="email"   placeholder="maria@example.com" />
          <Field label="Subject"       k="subject" placeholder="Research collaboration, data question, bug report…" />
          <Field label="Message"       k="message" placeholder="Tell us what's on your mind…" multi />
          <button onClick={() => { if(form.email && form.message) setSent(true); }}
            style={{ background:T.amber, color:"#000", border:"none", padding:"0.65rem 1.5rem",
              fontFamily:"DM Mono", fontSize:"0.6rem", fontWeight:500, letterSpacing:"0.1em",
              textTransform:"uppercase", cursor:"pointer", transition:"all 0.2s", marginTop:"0.5rem" }}
            onMouseEnter={e=>e.currentTarget.style.background="#fff"}
            onMouseLeave={e=>e.currentTarget.style.background=T.amber}>
            Send Message →
          </button>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"1rem" }}>
          {[
            ...
            <div key={i} style={{ padding:"1rem 1.25rem", border:`1px solid ${T.border}`,
              ...
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"1.3rem",
                color:c.color, opacity:0.5, lineHeight:1, flexShrink:0 }}>{c.icon}</div>
              <div>
                <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"0.85rem",
                  fontWeight:700, marginBottom:"0.2rem" }}>{c.title}</div>
                <div style={{ fontFamily:"DM Mono", fontSize:"0.58rem", color:T.muted, lineHeight:1.7 }}>{c.body}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ maxWidth:1200, margin:"0 auto",
        ...
        <div style={{ display:"grid",
          ...
          <div>
            ...
            <FLink label="Privacy Policy"      page="privacy" />
            <FLink label="Anonymity Framework" page="anonymity" />
