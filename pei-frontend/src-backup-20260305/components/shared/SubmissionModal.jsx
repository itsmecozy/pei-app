...
export default function SubmissionModal({ open, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.acknowledged) {
        setDone(true);
        if (onSuccess) onSuccess({ emotion: emotion.toLowerCase(), intensity, lgu_id: selectedLgu.id });
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.88)",
      ...
      <div style={{ background:T.surface, border:"1px solid "+T.border,
        ...
        <div style={{ padding:"1.25rem 1.5rem", borderBottom:"1px solid "+T.border,
          ...
          <button onClick={handleClose} style={{ background:"none", border:"none",
            color:T.muted, cursor:"pointer", fontSize:"1.1rem" }}>X</button>
        </div>

        <div style={{ padding:"1.25rem 1.5rem" }}>
          {done ? (
            <div style={{ textAlign:"center", padding:"1.75rem 0" }}>
              <div style={{ fontSize:"3rem", marginBottom:"0.75rem", animation:"successPop 0.5s ease" }}>o</div>
              <div style={{ fontFamily:"'Playfair Display',serif", fontSize:"1.1rem",
                fontWeight:700, marginBottom:"0.4rem" }}>You have been counted.</div>
              <p style={{ fontFamily:"DM Mono", fontSize:"0.62rem", color:T.muted, lineHeight:1.7 }}>
                Your feeling has been added to the national index. The Philippines felt you.
              </p>
            </div>
          ) : (
            <div>
              {step === 1 && (
                <div>
                  ...
                  <input type="text" value={query} onChange={e => setQuery(e.target.value)}
                    ...
                    autoFocus />
                  {searching && (
                    <div style={{ fontFamily:"DM Mono", fontSize:"0.56rem", color:T.muted, padding:"0.5rem 0" }}>Searching...</div>
                  )}
                  {lguResults.length > 0 && (
                    <div style={{ border:"1px solid "+T.border, maxHeight:200, overflowY:"auto", marginBottom:"0.5rem" }}>
                      {lguResults.map(lgu => (
                        <div key={lgu.id}
                  ...
                  {selectedLgu && (
                    <div style={{ background:T.amber+"18", border:"1px solid "+T.amber+"30",
                      padding:"0.6rem 0.7rem", fontFamily:"DM Mono", fontSize:"0.56rem", color:T.muted }}>
                      Selected: {selectedLgu.name}, {selectedLgu.province}
                    </div>
                  )}
                </div>
              )}
              ...
              {step === 2 && (
                <div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"0.35rem", marginBottom:"1.1rem" }}>
                    {EMOTIONS.map(em => (
                      <div key={em.name} onClick={() => setEmotion(em.key)}
                        style={{ border:"1px solid "+(emotion===em.key?em.hex:T.border),
                          ...
                          padding:"0.6rem 0.35rem", cursor:"pointer", textAlign:"center", transition:"all 0.2s" }}>
                  ...
                  <div style={{ position:"relative", height:36, display:"flex", alignItems:"center", marginBottom:"0.4rem" }}>
                    <div style={{ position:"absolute", width:"100%", height:3, background:"rgba(255,255,255,0.07)", borderRadius:2 }} />
                    ...
                    {[1,2,3,4,5].map(n => (
                      <button key={n} onClick={()=>setIntensity(n)}
                        style={{ position:"absolute", left:((n-1)/4*100)+"%",
                          ...
                          background:n<=intensity?T.amber:T.bg, cursor:"pointer", transition:"all 0.2s" }} />
                    ))}
                  </div>
                  <div style={{ display:"flex", justifyContent:"space-between", fontFamily:"DM Mono", fontSize:"0.5rem", color:T.muted }}>
        ...
        {!done && (
          <div style={{ padding:"0.9rem 1.5rem", borderTop:"1px solid "+T.border,
            ...
            <div style={{ display:"flex", gap:"0.3rem" }}>
              {[1,2,3].map(n=>(
                <div key={n} style={{ width:16, height:2, background:n<=step?T.amber:T.border, transition:"background 0.3s" }} />
              ))}
            </div>
              ...
              {step > 1 && (
                <button onClick={()=>setStep(s=>s-1)}
                  style={{ background:"none", border:"1px solid "+T.border,
                    ...
                    fontSize:"0.58rem", letterSpacing:"0.08em", textTransform:"uppercase", cursor:"pointer" }}>
                  Back
                </button>
              )}
              <button
                ...
                style={{ background:T.amber, color:"#000", border:"none",
                  ...
                  cursor:"pointer", opacity:submitting||(step===1&&!selectedLgu)||(step===2&&!emotion)?0.45:1 }}>
      ...
      <style>{"@keyframes modalIn{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}} @keyframes slideUp{from{transform:translateY(100%)}to{transform:translateY(0)}} @keyframes successPop{0%{transform:scale(0);opacity:0}60%{transform:scale(1.2)}100%{transform:scale(1);opacity:1}}"}</style>
    </div>
  );
}
